...
const createCatalogues = (option = {}) => {
  ...
  dirPaths.forEach((dirPath) => scannerMdFile(dirPath, option, node_path.basename(dirPath)));
...
const scannerMdFile = (root, option, prefix = "") => {
  ...
  const ignoreListAll = [...DEFAULT_IGNORE_DIR, ...ignoreList];
  const dirOrFilenames = node_fs.readdirSync(root);
  dirOrFilenames.forEach((dirOrFilename) => {
    ...
    if (node_fs.statSync(filePath).isDirectory()) {
      ...
      const { data: { catalogue, path = "" } = {} } = matter__default(content, {});
      if (catalogue && path) {
        const filename = node_path.basename(dirOrFilename, node_path.extname(dirOrFilename));
        catalogueInfo.push({
          filePath: `${prefix}/${filename}`,
          path,
          catalogues: createCatalogueList(node_path.join(srcDir, path), option, `/${path}/`)
        });
      }
    }
  });
...
const tryGetMarkdownTitle = (root, dirOrFilename) => {
  ...
  for (const filePath of filePaths) {
    ...
    const content = node_fs.readFileSync(filePath, "utf-8");
    const { data: { title } = {}, content: mdContent } = matter__default(content, {});
...
exports.default = VitePluginVitePressCatalogue;
